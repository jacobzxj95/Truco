...
bool AVT::PhysicsLogic::Init(	std::shared_ptr<flecs::world> _game, 
								std::weak_ptr<const GameConfig> _gameConfig)
{
	...
	game->system<CollisionSystem>()
		...
		queryCache.each([this](flecs::entity e, Mesh& me)
			{
			...
			/*for (int i = 0; i < polysize; ++i) {
				...
				polygon.poly[i].x += c.value.x;
				polygon.poly[i].y += c.value.y;
			}*/
			// sphere collider size
			GW::MATH::GSPHEREF temp = { me.t.row4.x, me.t.row4.y, me.t.row4.z, 0.9f };
			
			// add to vector
			testCache.push_back({temp,e});
		});
		...
		for (int i = 0; i < testCache.size(); ++i) {
			...
			for (int j = i + 1; j < testCache.size(); ++j) {
				...
				GW::MATH::GCollision::TestSphereToSphereF( 
					testCache[i].sphere, testCache[j].sphere, result);
...
bool AVT::PhysicsLogic::Activate(bool runSystem)
{
	if (runSystem) {
...
bool AVT::PhysicsLogic::Shutdown()
{
